...
Ollama (for the LLM backend). Download from ollama.com.
...
Scalability: In a production environment, bot_logic.py can be easily swapped to use OpenAI API (GPT-4), Google Gemini API, or Anthropic Claude simply by changing the endpoint. The core logic (Adaptive System Prompts) remains the same regardless of the model used.

The Sentiment Pipeline (sentiment_engine.py)
...
Windows Fix applied: A patch for torch.classes.__path__ is included to ensure stability on Windows environments.

The Orchestrator (bot_logic.py)
